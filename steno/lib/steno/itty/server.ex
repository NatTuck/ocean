defmodule Steno.Itty.Server do
  use GenServer
  ...
  def reg(uuid) do
    {:via, Registry, {Steno.Itty.Reg, uuid}}
  end
  ...
  def start(cmd, env, on_exit) do
    ...
    {:ok, _cpid} = DynamicSupervisor.start_child(Steno.Itty.DynSup, spec)
    {:ok, uuid}
  end

  def add_cookie(env, cookie) do
    if Map.has_key?(env, "COOKIE") do
      env
    else
      Map.put(env, "COOKIE", cookie)
    end
  end

  def to_charlist_pairs(mm) do
